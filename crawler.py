...
def crawl(do_hash: bool = True, hash_only: bool = False):
    ...
    et_ctx = exiftool.ExifToolHelper() if EXIFTOOL_AVAILABLE else None
...
def prune_excluded():
    ...
    Unlike --prune (rows for files gone from disk), this removes rows for files
    that still exist but now fall under an EXCLUDE_PATHS prefix — e.g. after
    adding a cache directory to the exclude list. It lets an exclude-list edit
    take effect retroactively, without a from-scratch recrawl. DuckDB never
    returns freed space to the OS, so follow this with a compact to shrink the
    file (the web UI's Compact DB button, or compact_db.py).

    Pure path-string matching against EXCLUDE_PATHS — the same component-aware
    predicate should_skip uses ('/x/FOO' never catches '/x/FOO_BAR') — so it
    needs no disk access and no mount guard: an excluded path is removed whether
    or not its volume is currently mounted.
    """
    print(f"\n{'='*60}")
    print(f"  Prune-excluded mode — removing rows now under the exclude list")
    print(f"  Database: {DB_PATH}")
    print(f"{'='*60}\n")
    ...
    # Per-entry impact for the user-editable list (the part most likely just
    # edited), shown before deleting so it's clear what each prefix removes.
    ...
    # Bulk delete over the full exclude set (defaults + user).
    conds, params = [], []
    ...
    print(f"\n{'='*60}")
    print(f"  Prune-excluded complete")
    print(f"  Before:   {before:,}")
    print(f"  Deleted:  {before - after:,}")
    print(f"  After:    {after:,}")
    print(f"  Compact the DB to reclaim the freed space.")
    print(f"  Database: {DB_PATH}")
    print(f"{'='*60}\n")
